...
def main():
    ...
    if not args.use_semantic:
        if args.subset=="all":
            item_tokens = [f"item_{i}" for i in range(18344)] # 22738
    ...
    print("Logical device index:", torch.cuda.current_device())
    print("Device name:", torch.cuda.get_device_name(torch.cuda.current_device()))
    print(f"Accelerate num_processes: {accelerator.state.num_processes}")
    print(f"Accelerate local_process_index: {accelerator.state.local_process_index}")
    print("-----------------------------------------------------------")
    ...
    model.train()
    device_id = accelerator.device # cuda
