...
def collate_fn(samples, pad_idx, eos_idx):
    ...
    # NOTE 여기 수정
    MAX_PATCH = 4  # 예시: 최대 4개로 맞춤
    for sample in samples:
        patches = sample["net_input"]["patch_images"]  # [N, 3, 224, 224]
        n_patch = patches.shape[0]

        if n_patch < MAX_PATCH:
            # 부족한 패치는 0으로 패딩
            pad = torch.zeros(MAX_PATCH - n_patch, 3, 224, 224)
            patches = torch.cat([patches, pad], dim=0)
        elif n_patch > MAX_PATCH:
            # 많으면 자름
            patches = patches[:MAX_PATCH]

        sample["net_input"]["patch_images"] = patches
