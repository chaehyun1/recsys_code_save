...
def data_partition(fname, path=None):
    ...
    if path == None:
        f = open('./sports/%s_sampling.txt' % fname, 'r') # NOTE: sampling된 데이터셋을 사용
    else:
        f = open(path, 'r')
    for line in f:
...
def evaluate(model, dataset, args):
    ...
    for u in users: # user id에 대해
        ...
        for _ in range(19): # negative item 19개 생성
        ...
        if rank < 1: # 예측된 아이템이 1위인 경우에만 점수 추가 
...
def evaluate_standard(model, dataset, args, split='test'):
    # 1. 데이터 준비
    [train, valid, test, usernum, itemnum] = copy.deepcopy(dataset)

    # 평가할 데이터와 이력(history) 설정
    if split == 'valid':
        data_to_evaluate = valid
        history_items = train
    else: # 'test'
        data_to_evaluate = test
        # 테스트 시에는 train과 valid 기록을 모두 합쳐서 history로 사용
        history_items = {u: train.get(u, []) + valid.get(u, []) for u in range(1, usernum + 1)}

    # NOTE: Recall, Precision 변수 추가
    NDCG = 0.0
    HT = 0.0
    RECALL = 0.0
    PRECISION = 0.0
    ...
    for u in users:
        if len(history_items.get(u, [])) < 1 or len(data_to_evaluate.get(u, [])) < 1:
            continue

        seq = np.zeros([args.maxlen], dtype=np.int32)
        idx = args.maxlen - 1
        for i in reversed(history_items[u]):
            seq[idx] = i
            idx -= 1
            if idx == -1: break

        rated = set(history_items[u])
        rated.add(0)
        pos_item = data_to_evaluate[u][0]
        item_idx = [pos_item]
        ...
        if rank < K:
            NDCG += 1 / np.log2(rank + 2)
            HT += 1
            # NOTE: Recall, Precision 계산 로직 추가
            RECALL += 1 # 정답이 1개이므로 Hit와 동일
            PRECISION += 1 / K # K개 중 1개를 맞췄으므로

        if valid_user % 100 == 0:
            print('.', end="")
            sys.stdout.flush()
            
    # NOTE: 최종 반환값에 Recall, Precision 추가
    return (
        NDCG / valid_user,
        HT / valid_user,
        RECALL / valid_user,
