...
image_dir = "/root/mlp_multi/sasrec_new/sports/image"
data_dir = f"/root/mlp_multi/{data_name}"
os.makedirs(f"{data_dir}/txt_features", exist_ok=True)

meta_data = []
with open("/root/mlp_multi/sasrec_new/sports/meta_Sports_and_Outdoors.json", 'r') as f:
    for line in f:
...
def random_sampling_noun_chunks(item_str):
    ...
    sample_size = max(1, int(len(tokens)*0.75))
    np.random.seed(123)
...
for i in tqdm(range(len(meta_data))):
    ...
    text_list.append(item_str_preprocissing)
