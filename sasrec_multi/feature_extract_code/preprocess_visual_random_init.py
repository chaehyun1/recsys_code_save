...
parser.add_argument("--data_name", type=str, default="sports_random_init")
...
meta_data = []
with open("/home/users/chaehyun/RS/A-LLMRec/pre_train/sasrec_multi/sports/meta_Sports_and_Outdoors.json", 'r') as f:
    for line in f:
        meta_data.append(json.loads(line))

image_folder = f"/home/users/chaehyun/RS/A-LLMRec/pre_train/sasrec_multi/sports/image"
...
for i in tqdm(range(len(meta_data))):
    ...
    if os.path.exists(txt_path):
        ...
        # masked_image = mask_random_patches(image, keep_ratio=1, patch=16, resize_size=(crop_size["width"], crop_size["height"]))
...
print("Encoding image feature (RANDOM INIT)")
emb_dim = vision_tower.config.hidden_size  # 예: 1152

for asin in tqdm(asin_list):
    rand_vec = torch.randn(emb_dim, dtype=torch.float32).to(torch.bfloat16)
    torch.save(rand_vec, f"{data_dir}/img_features/{asin}.pth")
